...
and can receive its ``initial_*`` and ``current_*`` slots from independent
write cycles (e.g. opening odds arrive at T-120, a later poll only refreshes
``current_odds`` at T-5). ``upsert`` merges into whichever slots have new
data instead of requiring both to arrive together.
...
from infrastructure.persistence.repositories.market.exchange_quote_payload import (
    SINGLE_SIDE,
)
...
class MarketChoiceQuoteWriter:
    ...
    def upsert(
        ...
        *,
        ...
        normalized_side = str(exchange_side or SINGLE_SIDE).strip().lower()
        ...
        quote = (
            ...
            .filter(
                ...
                MarketChoiceQuote.exchange_side == normalized_side,
