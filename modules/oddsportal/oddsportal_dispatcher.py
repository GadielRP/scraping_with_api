            ...
            try:
                for i, task in enumerate(tasks):
                    ...
                    try:
                        ...
                        match_url = None
                        if season_id:
                            match_url = scraper.find_match_url_from_cache(season_id, task['home_team'], task['away_team'], current_date=current_date, target_time_utc=task.get('starts_at'))
                        if not match_url:
                            match_url = await scraper.find_match_url(task['league_url'], task['home_team'], task['away_team'], season_id=season_id, current_date=current_date, target_time_utc=task.get('starts_at'))
                        ...
                        if data is None:
                            ...
                            if not retry_url and season_id:
                                retry_url = scraper.find_match_url_from_cache(season_id, task['home_team'], task['away_team'], current_date=current_date, target_time_utc=task.get('starts_at'))
                            if not retry_url:
                                retry_url = await scraper.find_match_url(task['league_url'], task['home_team'], task['away_team'], season_id=season_id, current_date=current_date, target_time_utc=task.get('starts_at'))
                            if retry_url:
                                retry_task = {
                                    "event_id": event_id,
                                    "match_url": retry_url,
