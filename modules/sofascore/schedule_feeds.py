...
def get_today_sport_events_response(client, date: str, sport: str, page: int = 1):
    ...
    return client.request_json_or_none(f"/sport/{sport}/scheduled-tournaments/{date}/page/{page}")
...
def get_today_sport_events_odds_response(client, date: str, sport: str):
    ...
    return client.request_json_or_none(f"/sport/{sport}/odds/1/{date}")


def get_live_events_response_per_sport(client, sport: str) -> Optional[Dict]:
    response = client.request_json_or_none(f"/sport/{sport}/events/live")
...
def get_unique_tournament_scheduled_events(client, unique_tournament_id: int | str, date: str):
    ...
    return client.request_json_or_none(f"/unique-tournament/{unique_tournament_id}/scheduled-events/{date}")
