"""M4 - Recent Inertia Engine module.
...
def calculate_recent_inertia_engine(
    ...
    """Calculate M4 - Recent Inertia Engine for an event."""
    del event_context
    ...
    m4_status, m4_status_reason = _determine_status(
    ...
    if home_gd_count < 3 or away_gd_count < 3:
        ...
        if m4_status == "ACTIVE":
            m4_status = "DEGRADED"
            m4_status_reason = "missing_or_partial_gd_sample"
    else:
    ...
    if m4_status == "INSUFFICIENT_DATA":
        m4_edge = 0.0
    ...
    if debug_mode:
        logger.info(f"--- M4 Recent Inertia Engine Debug: Event {event_id} ({participants}) ---")
        ...
        logger.info(f"  activation_status={m4_status} ({m4_status_reason})")
        ...
        # M4 Final Edge
        logger.info(
            f"  [M4_EDGE_RAW] ({_WEIGHT_ACTIVE_STREAK:.2f} * active_streak_edge({active_streak_edge:.12f})) + "
            ...
            f"({_WEIGHT_PERFORMANCE_FORM * performance_form_edge:.12f}) = {m4_edge_raw:.12f}"
        )
        logger.info(
