...
from infrastructure.settings.config import _parse_env_secret_list
from modules.oddspapi.client import OddsPapiClient, OddsPapiError
import modules.oddspapi.client as client_module
...
def test_api_key_list_supports_two_unique_comma_separated_keys(monkeypatch):
    monkeypatch.setenv("ODDSPAPI_TEST_KEYS", " key-1, key-2, key-1 ")

    assert _parse_env_secret_list("ODDSPAPI_TEST_KEYS") == [
        "key-1",
        "key-2",
    ]
...
def test_odds_endpoint_observes_configured_cooldown_from_response_completion(monkeypatch):
    OddsPapiClient._last_request_completed_at.clear()
