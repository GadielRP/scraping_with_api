...
def _candidate(
    *,
    ...
    return EventCandidateScore(
        ...
        participant1_primary_score=participant1_score,
        participant2_primary_score=participant2_score,
        participants_primary_score=participants_score,
...
def _event(
    *,
    ...
    return SimpleNamespace(
        id=event_id,
        starts_at=start_time.astimezone(timezone.utc),
        sport=sport,
        home_team=home_name,
        away_team=away_name,
...
def test_odds_response_does_not_run_layer3_matching(monkeypatch):
    monkeypatch.setattr(
        resolver_module.EventSourceMappingRepository,
        "get_event_id_by_source",
    ...
    monkeypatch.setattr(
        resolver_module.OddspapiEventCandidateMatcher,
        "find_best_match",
        lambda *args, **kwargs: pytest.fail("odds response must not run candidate matching"),
    )
    monkeypatch.setattr(resolver_module.db_manager, "get_session", _fake_session_context)
    ...
    assert resolution.resolved is False
    assert resolution.skipped_reason == "unresolved_no_direct_mapping"
...
def test_missing_sofascore_id_falls_through_to_layer3(monkeypatch):
    ...
    resolution = OddspapiEventResolver.resolve_from_fixture_response(_payload(external_providers={}))
...
def test_tennis_surname_first_names_match_canonical_given_name_order():
    fixture = OddspapiFixtureIdentity.from_payload(
        _payload(
            participant1_name="Duong, David",
            participant2_name="Potter, Emmett",
            sport_name="Tennis",
            tournament_name="UTR PTT Newport Beach Men 17",
            tournament_slug="utr-ptt-newport-beach-men-17",
            category_name="UTR Men",
            category_slug="utr-men",
        )
    )
    event = _event(
        event_id=121252,
        start_time=fixture.start_time_local,
        sport="Tennis",
        home_name="David Duong",
        away_name="Emmett Potter",
        competition_name="UTR PTT Newport Beach Men 17, Group B",
        category_name="UTR Men",
    )

    decision = OddspapiEventCandidateMatcher().find_best_match(
        fixture,
        session=_FakeSession([event]),
    )

    assert decision.resolved is True
    assert decision.canonical_event_id == 121252
    assert decision.best_candidate.participant1_score == 1.0
    assert decision.best_candidate.participant2_score == 1.0


def test_exact_identity_can_resolve_within_thirty_minute_time_drift():
    fixture = OddspapiFixtureIdentity.from_payload(
        _payload(
            participant1_name="Oak, Isabelle",
            participant2_name="Thornton, Maeve",
            sport_name="Tennis",
            tournament_name="UTR PTT Knoxville Women 02",
            tournament_slug="utr-ptt-knoxville-women-02",
            category_name="UTR Women",
            category_slug="utr-women",
        )
    )
    event = _event(
        event_id=121264,
        start_time=fixture.start_time_local + timedelta(minutes=20),
        sport="Tennis",
        home_name="Isabelle Oak",
        away_name="Maeve Thornton",
        competition_name="UTR PTT Knoxville Women 02, Group D",
        category_name="UTR Women",
    )
    ...
    assert decision.resolved is True
    assert decision.canonical_event_id == 121264
    assert decision.best_candidate.start_time_delta_minutes == 20.0
    assert decision.best_candidate.score < OddspapiEventCandidateMatcher.AUTO_LINK_THRESHOLD


def test_weak_nearby_candidate_does_not_make_exact_top_match_ambiguous():
    fixture = OddspapiFixtureIdentity.from_payload(
        _payload(
            participant1_name="Clitheroe FC",
            participant2_name="Morecambe FC",
            tournament_name="Club Friendly Games",
            tournament_slug="club-friendly-games",
            category_name="International Clubs",
            category_slug="international-clubs",
        )
    )
    exact_event = _event(
        event_id=121441,
        start_time=fixture.start_time_local + timedelta(minutes=15),
        home_name="Clitheroe FC",
        away_name="Morecambe",
        competition_name="Club Friendly Games",
        category_name="World",
    )
    weak_event = _event(
        event_id=121483,
        start_time=fixture.start_time_local,
        home_name="Cliftonville FC",
        away_name="Moyola Park AFC",
        competition_name="Club Friendly Games",
        category_name="World",
    )
    ...
    decision = OddspapiEventCandidateMatcher().find_best_match(
        fixture,
        session=_FakeSession([exact_event, weak_event]),
    )

    assert decision.resolved is True
    assert decision.canonical_event_id == 121441
    assert decision.second_best_candidate.both_teams_strong is False
